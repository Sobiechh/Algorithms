    ...
    while t <= generationCount:
        for particle in population:
            for j in range(dimension):
                alfa = random.random()
                beta = random.random()
                particle.velocity[j] = velocityRate * particle.velocity[j]                     + particlePosRate * alfa * (particle.bestPosition[j] - particle.position[j])                     + swarmPosRate * beta * (recentBestPosition[j] - particle.position[j])
